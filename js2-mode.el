...
(defvar js2-LCB 172)                    ; {|
(defvar js2-RCB 173)                    ; |}
(defvar js2-INTERFACE 174)              ; interface
(defvar js2-IMPLEMENTS 175)             ; implements
...
import ImportClause FromClause;"
  ...
  kind)      ; import type kind, e.g. import type or import typeof
...
(defun js2-print-function-node (n i)
  (let* ((pad (js2-make-pad i))
         ...
         (predicate (js2-function-node-predicate n))
...
(defun js2-print-type-params-node (n i)
  (let ((params (js2-type-params-node-params n))
        (trailing (js2-type-params-node-trailing n)))
    (insert "<")
    (when params
...
(cl-defstruct (js2-type-param-node
               (:include js2-node)
               (:constructor make-js2-type-param-node (&key (type js2-LT)
                                                            (pos js2-ts-cursor)
                                                            len
                                                            name
                                                            decl-p
                                                            type-annotation
                                                            init)))
  "AST node for type params param."
  name
  decl-p
  type-annotation
  init)

(js2--struct-put 'js2-type-param-node 'js2-visitor 'js2-visit-none)
(js2--struct-put 'js2-type-param-node 'js2-printer 'js2-print-type-param-node)

(defun js2-print-type-param-node (n i)
  (let ((name (js2-type-param-node-name n))
        (decl-p (js2-type-param-node-decl-p n))
        (ta (js2-type-param-node-type-annotation n))
        (init (js2-type-param-node-init n)))
    (js2-print-ast name 0)
    (when decl-p
      (when ta
        (js2-print-ast ta 0))
...
(cl-defstruct (js2-object-type-node
               (:include js2-node)
               (:constructor make-js2-object-type-node (&key type
                                                             (pos js2-ts-cursor)
                                                             len
                                                             properties
                                                             trailing
...
(cl-defstruct (js2-object-prop-type-node
               (:include js2-node)
               (:constructor make-js2-object-prop-type-node (&key (type js2-NAME)
                                                                  (pos js2-ts-cursor)
                                                                  len
                                                                  optional
                                                                  kind
...
(js2--struct-put 'js2-object-prop-type-node 'js2-printer 'js2-print-object-prop-type-node)
...
(cl-defstruct (js2-tuple-type-node
               (:include js2-node)
               (:constructor make-js2-tuple-type-node (&key (type js2-BITOR)
                                                            (pos js2-ts-cursor)
                                                            len
                                                            types
                                                            trailing)))
  "AST node for tuple type."
  types
  trailing)

(js2--struct-put 'js2-tuple-type-node 'js2-visitor 'js2-visit-none)
(js2--struct-put 'js2-tuple-type-node 'js2-printer 'js2-print-tuple-type-node)

(defun js2-print-tuple-type-node (n i)
  (let ((trailing (js2-tuple-type-node-trailing n))
        (types (js2-tuple-type-node-types n)))
    (insert "[")
    (when types
      (js2-print-ast (car types) 0)
...
(defun js2-print-generic-type-node (n i)
  (let ((name (js2-generic-type-node-name n))
        (params (js2-generic-type-node-params n)))
...
(cl-defstruct (js2-typeof-type-node
               (:include js2-node)
               (:constructor make-js2-typeof-type-node (&key (type js2-TYPEOF)
                                                             (pos js2-ts-cursor)
                                                             len
                                                             argument)))
  "AST node for typeof type."
  argument)

(js2--struct-put 'js2-typeof-type-node 'js2-visitor 'js2-visit-none)
(js2--struct-put 'js2-typeof-type-node 'js2-printer 'js2-print-typeof-type-node)

(defun js2-print-typeof-type-node (n i)
  (insert "typeof ")
  (js2-print-ast (js2-typeof-type-node-argument n) 0))
                          ...
                          (?<
                           ...
                           (if (js2-match-char ?<)
                             ...
                             (if (js2-match-char ?=)
                               ...
                               (throw 'return js2-LT))))
                          (?>
                           ;; TODO implement tokens scanner of type annotation
                           (if js2-in-type
                               (throw 'return js2-GT)
                             (if (js2-match-char ?>)
                                 (if (js2-match-char ?>)
...
(defun js2-parse-function-internal (function-type pos star-p &optional async-p name type-params predicate)
...
(defun js2-parse-function (function-type pos star-p &optional async-p name type-params predicate)
...
Support dynamic import sytnax, e.g. `import('foo')`."
  ...
  (let ((beg (js2-current-token-beg))
    ...
    (cond ((js2-match-token js2-STRING)
          ...
          (t
           (let* ((import-clause (js2-parse-import-clause kind))
  ...
  (let* ((beg (js2-current-token-beg))
    ...
    (cond
     ...
     ((js2-match-token js2-LC)
      (let ((imports (js2-parse-export-bindings t kind)))
...
(defun js2-parse-export-bindings (&optional import-p kind)
...
js2-LC. Return a lisp list of js2-export-binding-node"
  (let ((bindings (list)))
    (while
        (let ((binding (js2-maybe-parse-export-binding import-p kind)))
          (when binding
...
(defun js2-maybe-parse-export-binding (&optional import-p decl-kind)
...
(defun js2-parse-export (&optional kind begin)
...
invalid export statements."
  ...
  (let ((beg (if begin begin (js2-current-token-beg)))
        (children (list))
...
(defun js2-parse-assign-expr (&optional in-funcall in-conditional out-function)
  (let ((tt (js2-get-token))
    ...
    (if (= tt js2-YIELD)
      ...
      ;;   <T> () => {}
      ;; ---^
      ;; need parse type params first when matched `js2-LT'
      ;; FIXME: this will conflict with jsx as function call arguments
      ;; `js2-parse-argument-list' and conditional op ?:
      ;; 3. function param type annotation
      ;; troubled with function param type annotation too.
      ;;   (a: b) => {}
      ;; ----^
      ;; same as the return type, need parse type annotation.
      ;; 4. predicate annotation
      ;; should parse predicate with return type together.
      ;;   (): %checks
      ...
      (cond
       ...
       ((and (>= js2-language-version 200)
        ...
        (setq js2-recorded-identifiers recorded-identifiers
              js2-parsed-errors parsed-errors)
        (setq pn (js2-parse-function 'FUNCTION_ARROW (js2-current-token-beg) nil async-p nil type-params predicate)))
       (t
        (js2-unget-token)))
      pn)))
...
(defun js2-parse-type ()
  ...
  (let ((tt (js2-get-token))
    ...
    (cond
     ...
     ((or (= tt js2-LC)
          (= tt js2-LCB))
      (js2-parse-object-type tt))
     ;; function type, e.g. (a) => b
     ((= tt js2-LP)
      (js2-parse-function-type))
     ((= tt js2-LT)
      (let (type-params node)
        (js2-unget-token)
        (setq type-params (js2-parse-type-params t))
        (if (not (js2-match-token js2-LP))
            (js2-report-error "msg.syntax")
          (setq node (js2-parse-function-type))
          (setf (js2-function-type-node-type-params node) type-params)
          node)))
     ;; primitive type
     ((= tt js2-NAME)
...
(defun js2-parse-typeof-type ()
  "Parse typeof type, e.g. var a: typeof b;"
  (let ((pos (js2-current-token-beg))
        node)
    (setq node (js2-create-type-node t))
    (if (not node)
        (js2-report-error "msg.syntax")
      (make-js2-typeof-type-node :pos pos
                                 :len (- (js2-current-token-end) pos)
                                 :argument node))))
...
(defun js2-parse-type-params (&optional decl-p)
  ...
  (let ((pos (js2-current-token-beg))
        (continue t)
        params trailing)
    (if (not (js2-match-token js2-LT))
        (progn
          (js2-report-error "msg.syntax")
          params)
      (if (js2-match-token js2-COMMA)              ; match a<,
          ;; match a<, syntax error
          (progn
            (js2-report-error "msg.syntax")
            params)
        (if (js2-match-token js2-GT)               ; match a<>
            (make-js2-type-params-node :pos pos
                                       :len (- (js2-current-token-end) pos))
          (while continue
            ;; TODO decl-p, in type params state.
            (push (js2-parse-type-param decl-p) params)
            (if (js2-match-token js2-COMMA)
                (when (js2-match-token js2-GT)     ; match a<b,>
                  (setq continue nil
                        trailing t))
              (setq continue nil)
              (if (not (js2-match-token js2-GT))
                (js2-report-error "msg.syntax")))) ; end loop
          (make-js2-type-params-node :pos pos
                                     :len (- (js2-current-token-end) pos)
...
If param-decl-p was t, it will parse continue."
  (let ((pos (js2-current-token-beg))
    ...
    (if (not param-decl-p)
        (make-js2-type-param-node :pos pos
                                  :len (- (js2-current-token-end) pos)
                                  :name name)
      (when (js2-match-token js2-COLON)
        (let ((next-tt (js2-peek-token)))
          (if (or (= next-tt js2-COMMA)   ; match <a:,
                  (= next-tt js2-ASSIGN)  ; match <a:=
                  (= next-tt js2-GT))     ; match <a:>
              (js2-report-error "msg.syntax")
            (setq type-annotation (js2-create-type-node)))))
      (when (js2-match-token js2-ASSIGN)
        (if (= (js2-peek-token) js2-GT)   ; match <a = >
            (js2-report-error "msg.syntax")
          ;; TODO (Rabbit) <a = b<c: d>> should throw error.
          (setq init (js2-create-type-node t))))
      (make-js2-type-param-node :pos pos
                                :len (- (js2-current-token-end) pos)
                                :name name
                                :decl-p param-decl-p
...
(defun js2-parse-tuple-type ()
  "Parse tuple type, e.g. var v: [a, b]"
  (let ((pos (js2-current-token-beg))
        (continue t)
        types trailing)
    (if (js2-match-token js2-RB)                 ; match a[]
...
(defun js2-parse-function-type-param (&optional fst-p)
  ...
  (let ((pos (js2-current-token-beg))
        ...
        optional type-annotation name
        node)
    (if (not (or (= next-tt js2-COLON)
                 (= next-tt js2-HOOK)))
        (progn                                 ; not named, only the type annotation
          (js2-unget-token)
...
(defun js2-parse-module-kind ()
  ...
  (let ((kind (cond
    ...
    (when kind
      (let* ((next-tt (js2-get-token))
             (end-p (or (= next-tt js2-COMMA)
                        (and (= next-tt js2-NAME)
                             (string= (js2-current-token-string) "from"))
                        ;; match NamedImport when js2-ti-max-lookahead > 3 or 4
                        ;; import type as N from
                        ;; ____________1__2_3___
                        ;; and
                        ;; import type as as N from
                        ;; ____________1__2__3_4___
                        (and (>= js2-ti-max-lookahead 3)
                             (= next-tt js2-NAME)
                             (string= (js2-current-token-string) "as")
                             (let ((next-tt2 (js2-next-token))
                                   match-as)
                               (setq match-as
                                     (and (= next-tt2 js2-NAME)
                                          (not (string= (js2-current-token-string) "from"))
                                          (if (and (>= js2-ti-max-lookahead 4)
                                                   (string= (js2-current-token-string) "as"))
                                              (let ((next-tt3 (js2-next-token))
                                                    match-as-as)
                                                (setq match-as-as
                                                      (and (= next-tt3 js2-NAME)
                                                           (not (string= (js2-current-token-string) "from"))))
                                                (js2-unget-token)
                                                (not match-as-as))
                                            t)))
...
(defun js2-search-object (node name-node &optional class-p)
  ...
  ;; TODO(Rabbit): elem was one of `js2-object-prop-node' and `js2-method-node'
  (cl-loop for elem in (if class-p node (js2-object-node-elems node))
           for left = (cond
                       ((js2-object-prop-node-p elem) (js2-object-prop-node-left elem))
                       ((js2-method-node-p elem) (js2-method-node-left elem)))
...
i.e. ('name' 'value') = {name : { value: 3}}"
  (let (node
    ...
    (while (and temp names (js2-object-node-p temp-object))
      ...
      (and (setq node temp)
         (setq temp-object (cond
                     ((js2-object-prop-node-p temp) (js2-object-prop-node-right temp))
                     ((js2-method-node-p temp) (js2-method-node-right temp))))))
    (unless names node)))

(defun js2-search-node-name (node)
  "Find the node name."
  (let ((tp (js2-node-type node)))
    (cond
     ((js2-name-node-p node)
      (js2-name-node-name node))
     ((js2-class-node-p node)
      (js2-search-node-name
       (js2-class-node-name node)))
     ((js2-keyword-node-p node)
      (cond
       ((= tp js2-THIS)
        ;; TODO(Rabbit): fixed Me
        "Foo"))))))
...
and variables NAMES will contain one element."
  (let (node-init
        (val (js2-search-node-name (car names))))
    (setq node-init (js2-get-symbol-declaration node val))
    (when (> (length names) 1)
      ;; Check var declarations
      (when (and node-init (string= val (js2-search-node-name node-init)))
        (if (js2-class-node-p node-init)
            (let ((temp-names names))
              (pop temp-names)
              (setq node-init (js2-search-object
                               (js2-class-node-elems node-init)
                               (pop temp-names)
                               t)))
